...
export default function Task() {
    ...
    const listProjest = useSelector(state => state.task.all_project)
    const isLoadingListProjest = useSelector(state => state.task.is_loading_all_project)
    ...
    console.log("lis", listProjest)
    useEffect(() => {
        if (!isLoadingListProjest && listProjest) {
            let dataProject = []
            listProjest.map((project, i) => {
                const { id, project_name, project_description, departemen } = project
                return (
                    dataProject.push({
                        id: i + 1,
            ...
            setAllProject(dataProject)

        }

    }, [listProjest, isLoadingListProjest])
                ...
                <Grid item lg={10} style={{ backgroundColor: "whitesmoke", minHeight: "100vh" }}>
                    ...
                    <Box px={2} className="container-content" pb={5}>
                        <Grid container justifyContent="space-between">
                            ...
                            <Grid item lg={6}>
                                <Box>
                                    <Search placeholder="Search Project Name"
                                        onChange={(e) => {
                                            dispatch(getAllProject(e.target.value))
                                        }}
                                        allowClear
                                        style={{ width: 350 }} />
                                </Box>
                        ...
                        <Box textAlign="right" fontSize={12} pb={1} color="gray" >Total {allProject.length} Project</Box>
                        <Box pb={2}>
                            {isLoadingListProjest ? 'loading..' :
                                <Table columns={columns} dataSource={allProject} pagination />}
                        </Box>
                    </Box>
                </Grid>
            </Grid>
